...
+
The plugin SHALL provide a `variant_setup` fixture that returns a list of `VariantPluginBase` objects parsed from the `--variant-setup` option, using the same structure and parsing logic as regular variants.
+
...
pytest --variant=router:1.0,1.1,special\,edition --variant=switch:2.0,2.1,3.0
